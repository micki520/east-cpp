...
namespace lanms {
	...
	std::vector<Polygon> standard_nms(std::vector<Polygon> &polys, float iou_threshold) {
		...
		for(int i = 0; i < polys.size(); i++) {
			printf("---------------------print c++ standard_nms inmput -start------------------------ \n");
			printf("x1:%lf \n" , float(polys[i].poly[0].X) );
			printf("y1:%lf \n" , float(polys[i].poly[0].Y) );
			printf("x2:%lf \n" , float(polys[i].poly[1].X) );
			printf("y2:%lf \n" , float(polys[i].poly[1].Y) );
			printf("x3:%lf \n" , float(polys[i].poly[2].X) );
			printf("y3:%lf \n" , float(polys[i].poly[2].Y) );
			printf("x4:%lf \n" , float(polys[i].poly[3].X) );
			printf("y4:%lf \n" , float(polys[i].poly[3].Y) );

			printf("scores:%lf \n",polys[i].score);
			printf("---------------------print c++ standard_nms inmput -end-------------------------------- \n");
		}*/
		iota(std::begin(indices), std::end(indices), 0);   
